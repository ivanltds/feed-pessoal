...
export interface NewsPerspective {
  type: 'impact' | 'counterpoint' | 'outlook'
  badge: string
  title: string
  summary: string
}
  ...
  try {
    const response = await openai.chat.completions.create({
      ...
      messages: [
        {
          role: 'system',
          content: `Você é um analista sênior de inteligência de notícias.
Analise a notícia fornecida e gere EXATAMENTE 3 perspectivas analíticas em ${langName}.
